    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-display font-bold text-white">
                {projects?.filter(p => p.status === 'in_progress' || p.status === 'pending').length || 0}
              </h3>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card-strong p-6 rounded-2xl border border-[var(--color-glass-border)] relative overflow-hidden group">
          ...
          <h3 className="text-lg font-display font-bold text-white mb-2 relative z-10">Start a new project</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/dashboard/projects" className="text-sm text-[var(--color-text-muted)] hover:text-white transition-colors">View all</Link>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-display font-bold text-white">Recent Messages</h2>
            <Link href="/dashboard/messages" className="text-sm text-[var(--color-text-muted)] hover:text-white transition-colors">Open Chat</Link>
          </div>
              ...
              <div className="space-y-2">
                {messages.map(msg => (
                  <Link href="/dashboard/messages" key={msg.id} className="flex items-start gap-4 p-4 rounded-xl hover:bg-[var(--color-bg-glass-strong)] transition-colors group relative">
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-white">Surya CS</h4>
